...
#define kAIArtConverterSuiteVersion1	AIAPI_VERSION(1)
#define kAIArtConverterSuiteVersion		kAIArtConverterSuiteVersion1
...
typedef struct {
	...
	/**	Creates a new path or compound path that is an outline of the given artwork, and
		...
		the top element, possibly modified by stroke expansion, depending on the options.
   		...
   		*/
	AIAPI AIErr (*GetOutlineArt) ( AIArtHandle art, short paintOrder, AIArtHandle destination,
									long options, AIArtHandle *outlineArt );

	/* Converts an art object in place to a single path or compound path, disposing of the original.
			@param art The art object.
			@param convertToOutlineOptions Option flags for conversion, a logical OR of these values:
					<br> \c #kOutlineExpandAppearance
					<br> \c #kOutlineEliminateUnpainted
					...
					<br> \c #kOutlineAlwaysStrokeStraightLines
		*/
	AIAPI AIErr (*ArtConvertToOutline) ( AIArtHandle *art, long options );
