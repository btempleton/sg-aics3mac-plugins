...
typedef struct {
	...
	/** Reports whether a group is a clip group. Use \c #AIArtSuite::GetArtUserAttr()
		to determine which members of the group are clipping objects.
			@param group The art group object.
			@param clipped [out] A buffer in which to return true if the group
				contains any members that are clipping objects.
		*/
	AIAPI AIErr (*GetGroupClipped) ( AIArtHandle group, AIBoolean *clipped );

	/** Turns clipping on or off for all members of a group.  Use
		\c #AIArtSuite::SetArtUserAttr() to set or clear \c #kArtIsClipMask for
		individual contained paths.
			@param group The art group object.
			@param clipped True to turn clipping on, false to turn clipping off.
	*/
