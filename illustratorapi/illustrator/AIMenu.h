...
enum  {
	...
	kMenuGroupAddAboveNearGroup			= (1L<<4)
};
...
/**	@ingroup Suites
	...
	and to make simple applearance changes automatically based on the changes in
	artwork.
	...
	\li General pre- and post-menu notifiers, \c #kAIMenuBeforePluginNotifier
		and  \c #kAIMenuAfterPluginNotifier, are sent when the user selects
		a plug-in menu item.
	...
	\li The pre- and post-menu notifiers for Illustrator commands do not have
		...
		The menu identifiers are given in the \c #MenuCommands array,
		defined in \c AIMenuCommandNotifiers.h.

	\li When a user selects a menu item that is not in the \c MenuCommands list
		and is not a plug-in menu item, generic notifiers
		\c #kAIMenuBeforeUnknownMenuNotifier and \c #kAIMenuAfterUnknownMenuNotifier
		are sent.
	...
	*/
	...
	/** Retrieves the platform-specific menu item reference associated with a menu item.
			...
			@param platformMenuItem [out] A buffer in which to return the
				platform-specific object. The \c #AIPlatformMenuHandle
	...
	/** Updates a menu item's appearance automatically when specified conditions
		...
		positive result, which is true if any of the positive criteria are true.
			\li The results of the negative critera are combined to produce a
		negative result, which is true if none of the negative critera are true.
	...
	/** Retrieves a reference to the platform-specific menu structure for a menu
		with the range of items that belong to a menu group.
			...
			@param theMenu [out] A buffer in which to return the platform-specific menu information.
				The \c #AIPlatformMenuHandle is a \c MenuInfo structure in Mac OS,
