...
#define kAIColorSpaceSuiteVersion		AIAPI_VERSION(5)
/** Color space suite version */
...
typedef struct {
	...
	/** Reports the number of color space objects in a color-space list.
		...
		*/
	AIAPI int (*GetNumColorSpaces) (const ai::ColorSpaceList &csList);
	/**	Retrieves a color space object from a color-space list.
		...
		*/
	AIAPI AIErr (*GetNthColorSpace) (const ai::ColorSpaceList &csList, int n, ai::ColorSpace &cs);
	...
	/** Normalizes a color space list by flattening any NChannel color space,
		...
		*/
	AIAPI AIErr (*Normalize)(ai::ColorSpaceList& csList, short docColorModel, AIBoolean convertGrayToFullProcess);
	...
	/** Creates a process-separation CMYK color space.
			...
			@param hasAlpha True to create a color space with an alpha channel, false otherwise.
		*/
	AIAPI AIErr (*NewProcessSeparationColorSpace) (ai::ColorSpace &cs, int separationNum, AIBoolean hasAlpha);
	...
	*/
	AIAPI AIErr (*NewIndexedColorSpace) (ai::ColorSpace &cs, const ai::ColorSpace &baseCs, int numComponents,
		                           int numEntries, const ASUInt8 *lookup);
	...
	*/
    AIAPI int (*GetNumComponents) (const ai::ColorSpace &cs);
	/** Reports the number of color components in a color space. Does not count the alpha channel, if any.
		...
		*/
    AIAPI int (*GetNumColorComponents) (const ai::ColorSpace &cs);
	/** Reports the number of spot components in a color space. Does not count the alpha channel, if any.
		...
		*/
	AIAPI int (*GetNumSpotComponents) (const ai::ColorSpace &cs);
	...
	/** Reports the number of components in an indexed color table.
		...
		*/
	AIAPI int (*GetIndexedNumComponents) (const ai::ColorSpace &cs);
	...
	*/
	AIAPI int (*GetIndexedNumEntries) (const ai::ColorSpace &cs);
	...
	/** Retrieves the alternate color values in the alternate color space for a separation color space.
		...
		*/
	AIAPI const AIReal* (*GetSeparationAltColors) (const ai::ColorSpace &cs);
	...
	*/
	...
	/** Retrieves the colorant name of a component of a color space. Use with
		...
		*/
	AIAPI AIErr (*GetNthColorant) (const ai::ColorSpace &cs, int n, ai::UnicodeString& name);
