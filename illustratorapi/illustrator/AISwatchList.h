...
#define kAISwatchGroupSuiteVersion1			AIAPI_VERSION(1)
#define kAISwatchGroupSuiteVersion			kAISwatchGroupSuiteVersion1
...
typedef struct {
	...
	/** Reports the number of swatch groups in a swatch list. There is always at
		...
		(Note that this function returns an integer value, not an error code.)
			...
			*/
	int ASAPI (*CountSwatchGroups) (AISwatchListRef list);
	...
	/** Retrieves a swatch group from a swatch list.
	 	...
	 	*/
	AISwatchGroupRef (*GetNthSwatchGroup) (AISwatchListRef list, int index);
	...
	ASErr ASAPI (*NewSwatchGroup) ( AISwatchListRef list, AISwatchGroupKind groupKind,
									int atIndex, AISwatchGroupRef *newGroup );
	...
	/** Reports whether a swatch group is in a swatch list.
		...
		*/
	ASBoolean ASAPI (*SwatchGroupIsInList) (AISwatchListRef list, AISwatchGroupRef swatchGroup, int *indexFoundAt ); 
	...
	/** Removes a swatch group from a swatch list by index position. You cannot remove
		 the general swatch group (index 0). The index positions of all later groups are decremented.
			...
			@param deleteSwatches When false, moves the member swatches into the general group,
				...
				<br> When true, deletes the member swatches along with the group.
				If the swatch is a custom color swatch and it contains the last reference
				to that global color in the swatch list, then the custom color is also
				deleted from the document and any usages replaced by the equivalent process color.
		...
		*/
	ASErr ASAPI (*RemoveNthSwatchGroup) (AISwatchListRef list, int index, ASBoolean deleteSwatches );
	...
	/** Changes the position of a swatch group specified by index within a swatch list.
	  ...
	  */
	ASErr ASAPI (*ReorderNthSwatchGroup) (AISwatchListRef list, int fromIndex, int newIndex );
	...
	/** Changes the position of a swatch group specified by reference within a swatch list.
	  ...
	  */
	ASErr ASAPI (*ReorderSwatchGroup) (AISwatchListRef list, AISwatchGroupRef swatchGroup, int newIndex );
	...
	/** Moves a swatch to a new position within a swatch group.
 		...
 		*/
	ASErr ASAPI (*ReorderSwatch) (AISwatchGroupRef swatchGroup, AISwatchRef swatch, int toIndex );
	...
	/** Moves a swatch to a new position within a swatch group.
 		...
 		*/
	ASErr ASAPI (*ReorderNthSwatch) (AISwatchGroupRef swatchGroup, int fromIndex, int toIndex );
	...
	/** Reports the number of swatches in a swatch group.
		...
		*/
	int ASAPI (*CountSwatches) (AISwatchGroupRef swatchGroup);
	...
	/** Retrieves a swatch from a swatch group by position index.
	  ...
	  */
	AISwatchRef ASAPI (*GetNthSwatch) (AISwatchGroupRef swatchGroup, int index);
	 ...
	 */
	AISwatchRef ASAPI (*InsertNthSwatch) (AISwatchGroupRef swatchGroup, AIColor *aicolor, int index);
	...
	/** Removes a swatch specified by index position from a swatch group.
		...
		*/
	ASErr ASAPI (*RemoveNthSwatch) (AISwatchGroupRef swatchGroup, int index, ASBoolean deleteCustomColor);
	...
	/** Retrieves the group of a specific swatch in a swatch list.
		...
		*/
	ASErr ASAPI (*LocateSwatch) (AISwatchListRef list, AISwatchRef swatch, AISwatchGroupRef *containingSwatchGroup, int *groupIndex, int *swatchIndex);
	...
	/** Moves a swatch from one group to another within the same list. Other index positions within
		...
		*/
	ASErr ASAPI (*ReorderSwatchBetweenGroups) ( AISwatchGroupRef fromGroup, int fromIndex, AISwatchGroupRef toGroup, int toIndex );
