...
#define kAIFileFormatSuiteVersion10		AIAPI_VERSION(10)
#define kAIFileFormatSuiteVersion		kAIFileFormatSuiteVersion10
...
/** @ingroup Selectors
	...
	@note Handle this or the legacy version, \c kAI110SelectorAIUpdateFileFormat,
		not both.
	   */
...
typedef struct FileFormatExtData {
	/** The type of union (none or placement request). */
	ASInt32 m_lExtType;
...
struct AIFileFormatMessage {
	...
	private:
	/** @deprecated The file to read, write or check, deprecated in favor of \c filePath,
	    but remains in this structure for backwards binary compatibility up to AI11. */
	...
	/** Additional information required for specific operation (currently
		only for placement). Used if the plug-in registered
...
enum eLinkUpdateType {
	/** Perform a normal update. Currently unused. */
	kNormalUpdate,
