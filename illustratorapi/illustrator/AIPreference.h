...
typedef struct AIPreferenceSuite {
	...
	/** Retrieves a file specification preference.
			@param prefix The name of the plug-in, or \c NULL for an application preference.
			@param suffix The name of the preference.
			@param value [out] A buffer in which to return the value.
		*/
	...
	/** Sets a file specification preference.
			@param prefix The name of the plug-in, or \c NULL for an application preference.
			@param suffix The name of the preference.
			@param value The new value.
		*/
	...
	/** Adds a panel to the Preferences dialog, using a dialog you define to
		...
		to the Preferences submenu, creating and returning the menu item object.
			...
			@param prefItemGroup [out] A buffer in which to return the new dialog item-group
				...
				with \c #GetPreferencePanelItemRef() and \c #GetPreferencePanelBaseItemIndex()
	...
	/**  Retrieves the base ADM item object (the panel parent) of your Preferences panel.
	  	 Use with \c #GetPreferencePanelBaseItemIndex() to access or iterate through all the
	  	 UI items in the panel, using ADM functions to update the UI to reflect
	  	 the correct information when the panel is shown, or to retrieve input
	  	 from the user. The call is only valid when the Preferences dialog is displayed.
			@param prefItemGroup The panel object, as returned by \c #AddPreferencePanel().
			@param itemRef [out] A buffer in which to return the item reference.
			@see \c #ADMItemSuite9
		*/
	...
	/** Retrieves the index position of the base item from a preference item group.
		...
		Use with \c #GetPreferencePanelItemRef() to access individual UI items in the panel.
