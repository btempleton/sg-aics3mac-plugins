...
/**	@ingroup Suites
	...
	caller must decrement the count when that entry is no longer needed, to
	avoid memory. For convenience, the "To<Type>" and \c #AIDictionarySuite::Set()
...
result = sAIDictionary->Set(dict, "MyKey", sAIEntry->FromReal(15));
	...
	The \c #GetType() function does not consume the parameter; use it to determine the
