...
enum AIPluginGroupOptions {
	...
	/** If off (the default), when a user edit removes all objects from the edit group,
		the whole plug-in group object is deleted (standard behavior for all groups)
		If on, the plug-in group remains in the artwork	after its edit group has been emptied.
		Use for objects that build their result art based entirely on data and external context. */
	...
	/**	If on, the plug-in group is not targeted when fully selected. Users must
		target the plug-in group through the Layers palette.
	...
	/** Together with \c #kPluginGroupBlendAsPluginGroup, defines how plug-in groups
		...
		the Make Blend command is disabled. Also, plug-in groups with this option on
		do not respond to clicks of the Blend tool.
	...
	/** If on, paint styles applied to content of this plug-in group should not be obtained or set
		...
		receives the \c #kSelectorAICollectPaintStyles message when Illustraator would normally
...
/** @ingroup Errors
	Return code for response to \c #kSelectorAINotifyEdits
	at \c #kAfterOperationTime, with \c #kReadOperationCode,
	if the plug-in group should be destroyed.  */
...
/** Sent only at \c #kBeforeOperationTime (after an Expand, the object no longer exists).
	The plug-in art object is about to be expanded, that is, replaced by its result art
	contents. Remove from the result art descendants any tags or other state
	specific to the plug-in, and remove from the plug-in's global data anything
	that tracks this art object. */
...
/** Lock/unlock or show/hide. See \c #AIAttributeOperationSubcodes for subcode values. */
...
typedef struct {
	...
	/** For \c #kSelectorAINotifyEdits, supplied if \c code is \c #kTransformOperationCode
		...
		The matrix used for the tranformation. */
	AIRealMatrix		matrix;
...
typedef struct {
	...
	/**	Transformation options, a logical OR of \c #AITransformArtOptions.
		...
		\c #kTransformStrokePatterns, or \c #kScaleLines is on. */
	long				transformFlags;
...
@code
sAIArtNewArt( kPluginArt, kPlaceAboveAll, NULL, *pluginGroupArt );
sAIPluginGroupUseAIPluginGroup (pluginGroupArt, gmyPluginGroupHandle );
@endcode

	This activates the plug-in group art and allows you to access
	it’s edit and result groups.
