...
#define kAIHitTestSuiteVersion12	AIAPI_VERSION(3)
#define kAIHitTestSuiteVersion		kAIHitTestSuiteVersion12
...
/** Hit requests specify what types of hits to report. */
enum AIHitRequest {
...
/** A hit object, which describes what was found by
	\c #AIHitTestSuite functions at a specified point.
	Contains both generic information that applies to all objects,
	and information specific to the object type. For example, it always identifies
	the object if one was found. If the object is a path, there is
	additional information about the part of the path that was hit.
	See specific suites, such as \c #AIMeshHitSuite,
	to extract object-specific information.
	*/
...
typedef struct {
	...
	AIArtHandle group;
	/** The exact point location of the hit.
...
typedef struct {
	/** Finds an object in a given art tree at a given point.
		Searches from front to back in the paint order, within a small
		tolerance of the given point. Returns the first object found that
		matches the criteria specified by \c option.
	...
	/** Finds an objectin a given art tree at or near a given point. Differs from \c #HitTest()
	...
	/** Finds an object at a given point, performing custom hit testing for
		plug-in objects, as defined by the plug-in group.
			...
			*/
	AIAPI AIErr (*CustomHitTest) ( AIArtHandle art, AIRealPoint *point, AIReal tolerance, long option,
