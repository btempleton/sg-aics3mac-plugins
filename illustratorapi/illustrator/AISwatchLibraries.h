...
#define kAISwatchLibrariesSuiteVersion	AIAPI_VERSION(3)
...
typedef ASUInt32 AISwatchLibraryType;
...
typedef struct
{
	...
	/** Retrieves a swatch list from a swatch library. Opens the library file and reads
		...
		*/
	ASErr ASAPI (*OpenNthLibrary)(long num, ASBoolean showPalette, AISwatchListRef *swatchList );
	...
	*/ 
	ASBoolean ASAPI (*NthLibraryIsOpen)( long num );

	/** Selects a swatch in the main Swatches palette, if it is open and the swatch is shown.
		(Note that this function returns a boolean value, not an error code.)
		No error codes are returned; invalid indices simply return false.
			@param index The 0-based position index into the main document's full swatch list
				(including any swatches that are currently filtered from the view), or a negative
				number to select the last ungrouped swatch in the document.
			@param deselectOthers True to also deselect any other selected swatches or color groups
				if the specified swatch is visible. (Do not pass an invalid index
				or the index of a non-visible swatch in order to deselect all.)
			...
			@note This is a palette selection only; it does not apply the selected swatch's color to
				the selected art. It is intended to be used by plug-ins that create swatches,
				to select the swatches they have just inserted into the list.
	*/
	ASBoolean ASAPI (*SelectNthDocumentSwatchIfVisible) ( int index, ASBoolean deselectOthers );
	...
	*/
	ASBoolean ASAPI (*SelectNthDocColorGroupIfVisible) ( int index, ASBoolean deselectOthers );
	...
	/** Rebuilds the libraries menu, populating it with the current list of libraries.
		...
		*/
	ASErr ASAPI (*BuildLibrariesList) ( ADMHierarchyListRef librariesList, AISwatchLibraryType includeLibraryTypes, int startIndex, long *contentChangeStampRef, int *checkLibraryID);
	...
	*/
	ASErr ASAPI (*GetActiveSwatchList) ( ADMHierarchyListRef librariesList, AISwatchListRef *swatchlist, int *activeLibraryID);
	...
	/** Counts the total number of selected swatches in the main document Swatches palette. Use this function
		...
		*/
	int ASAPI (*CountSelectedDocSwatches) ( );
	...
	/** Retrieves an array of colors for the selected document swatches. The
		...
		*/
	ASErr ASAPI (*GetSelectedDocSwatchColors) ( AIColor *colors, int bufferSize );
	...
	/** Counts the total number of selected swatch groups in the document Swatch palette. Use
		...
		*/
	int ASAPI (*CountSelectedDocSwatchGroups) ( );
	...
	/** Returns an array of the document's selected swatch groups.
		...
		*/
	ASErr ASAPI (*GetSelectedDocSwatchGroups) ( AISwatchGroupRef *swatchGroups, int bufferSize );
	...
	*/
	ASErr ASAPI (*GetRelativeDocSwatch) ( AIRelativeDocSwatchKind selectKind, AIBoolean selectIt, AIBoolean wrap,
