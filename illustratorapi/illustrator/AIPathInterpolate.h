...
typedef struct {
	...
	/**	Calculates the default number of steps for a smooth blend between two paths, based
    	primarily on the maximum distance in any color channel between the two paint
    	styles.
    		@note If the objects are not appropriate for shading,
				...
				shading (such as type objects, groups and symbols), the calculation
				...
				@see \c #SmoothColorBlendStepsAlongPath()
    		@param startPath The starting path.
    		@param endPath The ending path.
			@param densityLimit When positive, calculates the maximum distance from a bounding
    			...
    			and takes this as the maximum number of paths per point to place in that distance.
    			For example, if the maximum distance between the two blended
      			paths is 4 points, and the density limit is 8, then steps are at most 32
      			no matter how different the colors are. Use to avoid many paths crowded into
      			a tiny distance, when the blend would look the same with a much smaller number of paths.
	...
	/** Calculates the default number of steps for a smooth blend between two paths,
		for cases where the objects are not appropriate for smooth color
		spacing. The calculation is based on the separation between objects,
		so that the space between them is filled with objects that almost overlap
		or barely overlap.

		Specifically intended for blends along non-default spline paths.

		Can also be used where there is no color channel difference (the objects
		are painted with the same color), or where objects are painted with multiple colors
		(such as gradients and patterns), or have effects or outlines
		too complex for shading (such as type objects, groups and symbols)

			 @see \c #SmoothColorBlendSteps()
 			...
 			@param densityLimit When positive, calculates the maximum distance from a bounding
 				...
 				Ignored if zero or negative.
			@param steps [out] A buffer in which to return the number of steps.
			@param pathDistance Used to calculate the separation, instead of the object bounds.
				Also used in the density limit calculations.
		*/
