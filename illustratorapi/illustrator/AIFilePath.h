...
#define kAIFilePathSuiteVersion		AIAPI_VERSION(2)
...
struct AIFilePathSuite
{
	...
	*/
	...
	/** Sets a file path from a file specification.
			...
			@param path The file path object.
	*/
	void AIAPI (*SetFromSPFileSpec)(const SPPlatformFileSpecification&, ai::FilePath &path);
	...
	*/
	...
	/** Sets a file path from an \c FSSpec file specification
		(in Mac OS only, outside Illustrator core).
			...
			@param path The file path object.
	*/
	AIErr AIAPI (*SetFromFSSpec)(const FSSpec&, ai::FilePath &path);
	...
	*/
	AIErr AIAPI (*GetAsSPPlatformFileSpec)(const ai::FilePath &path, SPPlatformFileSpecification &spPlatformFileSpec);
	...
	/** Retrieves a file path as an \c FSSpec file specification (in Mac OS only, outside Illustrator core).
		(Note that this function returns an OS status value, not an error code.)
			@param path The file path object.
			@param fsSpec [out] A buffer in which to return the file specification.
			@return The OS status.
	*/
	OSStatus AIAPI (*GetAsFSSpec)(const ai::FilePath &path, FSSpec &fsSpec);
