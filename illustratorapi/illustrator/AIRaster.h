...
#define kAIRasterSuiteVersion8	AIAPI_VERSION(8)
#define kAIRasterSuiteVersion	kAIRasterSuiteVersion8
...
typedef struct AIRasterSuite {
	...
	/** Post-concatenate a transformation onto the matrix applied to a placed object.
			@param raster An art object of type \c #kRasterArt.
    ...
    AIAPI AIErr (*GetOverprint) ( AIArtHandle raster, AIBoolean *status );
 	...
 	/** Decomposes a raster object into an array of rasters specified in the standard
 		...
 		*/
     AIAPI AIErr (*SplitChannels) ( AIArtHandle raster, AIArtHandle **rasterArray, long *numRasters, uint32 flags );
 	...
 	/** Interleaves the color channels from an array of raster objects and
 		...
 		channels.
			...
			@see \c #SplitChannels()
 		*/
     AIAPI AIErr (*InterleaveChannels) ( AIArtHandle *rasterArray, long numRasters, AIArtHandle *raster, uint32 flags );
