...
#define kAIPathStyleSuiteVersion9	AIAPI_VERSION(9)
#define kAIPathStyleSuiteVersion	kAIPathStyleSuiteVersion9
...
struct AIDashStyle {
	...
	AIReal offset;
	/** Dash array */
	AIReal array[kMaxDashComponents];
};
...
struct AIPathStyleSuite {
	...
	/** Converts an \c #AIColor to a pre-version-8 format.
			@param from	A pointer to the \c #AIColor to convert.
			@param to [out] A buffer in which to return the converted color.
		*/
	AIAPI AIErr	(*AIColorToAI70Color) ( AIColor *from, AI70Color *to);

	/** Converts a pre-version-8 format color to an \c #AIColor.
			@param from	A pointer to the early-format color to convert.
			@param to [out] A buffer in which to return the converted color.
		*/
