...
AIErr createFindReplaceDialog( AINotifierMessage *message ) {
	...
	g->FindReplaceDialog = sADMDialog->Create( message->d.self,"Find & Replace Graphics", kADMFindReplaceDialogID,
										   kADMTabbedFloatingDialogStyle, FindReplaceDlgInit, nil, 0 );
...
ASErr ASAPI FindReplaceDlgInit(ADMDialogRef dlg)
{
	...
	sADMDialog->SetDestroyProc( dlg, FindReplaceDlgDestroyProc );
...
//	if ( ApplyTo == 0 ) {	controlFlags = kVisitColorsDirectOnly | kVisitColorsStrokesOnly | kVisitGlobalObjectsOnceOnly;  }
//	if ( ApplyTo == 1 ) {	controlFlags = kVisitColorsDirectOnly | kVisitColorsFillsOnly | kVisitGlobalObjectsOnceOnly;  }
//	if ( ApplyTo == 2 ) {	controlFlags = kVisitColorsDirectOnly | kVisitGlobalObjectsOnceOnly;  }
	...
	for ( int i=0 ; i < count ; i++ ) {
		...
		if ( sADMEntry->GetIndex(sADMList->GetActiveEntry(sADMItem->GetList(ghAttributeList))) == 0) {
			...
			//sAIPathStyle->ReplaceObjectAIColor(currArtObj,&FromToColors.FromColor ,&FromToColors.ToColor, false, NULL);
