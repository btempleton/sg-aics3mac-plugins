...
AIErr createAddColorDialog( AINotifierMessage *message ) {
	...
	g->AddColorDialog = sADMDialog->Create( message->d.self,"Add Color", kADMAddColorDialogID,
										   kADMTabbedFloatingDialogStyle, AddColorDlgInit, nil, 0 );
...
ASErr ASAPI AddColorDlgInit(ADMDialogRef dlg)
{
	...
	sADMDialog->SetDestroyProc( dlg, AddColorDlgDestroyProc );
	...
	ghEditTextItemRef = sADMItem->Create(dlg, kADMUniqueItemID, kADMTextEditType, &boxloc, NULL, NULL, NULL);
	...
	if ( sADMNotifier->IsNotifierType( notifier, kADMUserChangedNotifier ) )
	{
		...
		if ( swatchName == ai::UnicodeString("MICR") ) {
			...
			if (newSwatch) {
				...
				swatchName = ai::UnicodeString(MICR_BLACK_MAG_COLOR_NAME);
				...
				//Make the custom color using these attributes, the color name,
		...
		else if ( swatchName.compare( (ai::UnicodeString)"0" ) ) {
			
			//If the name matches one in the book, store the attributes in customColor
			//if ( sAISwatchLibrary->FindLegacyBookColor(swatchName, &customColor) ) {
