...
static AIErr StartupPlugin ( SPInterfaceMessage *message ) {
	...
	if ( ! IsPluginPathLocal() ) {
		sADMBasic->MessageAlert("The additional plugins search path must be on your desktop. Plugins have not been loaded.");
		error = 1;
		goto error;
	}
	...
	/*	//REGISTER THE ACTION EVENT
	 if (sAIActionManager) {
	 AIActionUserData actionUserData;
	 AIActionParamTypeRef tpb;
	 sAIActionManager->AINewActionParamType(&tpb);
	 //		sAIActionManager->RegisterActionEvent("btFixBlack",		//Global name
	 //											  "Fix Black",		//Local name
	 //											  NULL,				//callback
	 //											  1,				//version (?)
	 //											  0,				//flag    (?)
	 //											  message->d.self,	//this plugin
	 //											  actionUserData, //void pointer to user data (I don't think we're using this for this one)
	 //											  tpb);				//the parameter types (which is empty for this one)
	 sAIActionManager->AIDeleteActionParamType(tpb);
	 } else {
	 sADMBasic->MessageAlert("Action Manager is not loaded.");
	 error = 1;
	 goto error;
	 }
	 */
