...
//	long isTrue;
	...
	{
		sAIMenu->SetItemText( message->menuItem, ai::UnicodeString("Hide Non-Modal Dialog") );
	}
	else
	{
		sAIMenu->SetItemText( message->menuItem, ai::UnicodeString("Show Non-Modal Dialog") );
	}
...
extern AIErr addMenu( SPInterfaceMessage *message ) {
	...
	const char *menuGroupCStr = "SDKWindowGroup";
	const char *menuCStr = "SDK";
	const char *pluginCStr = "ADM Non Modal Dialog";
