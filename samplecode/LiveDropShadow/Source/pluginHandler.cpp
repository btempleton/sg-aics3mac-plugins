//========================================================================================
//  
//  $File: //ai/ai13/devtech/sdk/public/samplecode/LiveDropShadow/Source/pluginHandler.cpp $
//
//  $Revision: #2 $
//
...
AIErr handlePluginCaller( char* caller, char* selector, void *message ) {
	...
	if ( sSPBasic->IsEqual( caller, kCallerAIMenu ) ) {
			 if ( sSPBasic->IsEqual( selector, kSelectorAIUpdateMenuItem ))
				error = goEnable( (AIMenuMessage *) message );
		
			 else if ( sSPBasic->IsEqual( selector, kSelectorAIGoMenuItem )) 
	...
	if ( sSPBasic->IsEqual( caller, kCallerAIPluginGroup ) ) {
		if ( sSPBasic->IsEqual( selector, kSelectorAINotifyEdits ) )
			error = goNotify( ( AIPluginGroupMessage* )message );
		else if ( sSPBasic->IsEqual( selector, kSelectorAIUpdateArt ) )
			error = goUpdate( ( AIPluginGroupMessage* )message );
	}			
#endif
